...
# **2. [LET’S VERIFY STEP BY STEP](https://proceedings.iclr.cc/paper_files/paper/2024/file/aca97732e30bcf1303bc22ac3924fd16-Paper-Conference.pdf) (26-6-9)**

这篇文章是OpenAI发表的一篇高引用的系统分析论文，该论文通过严谨、大规模的实验证明了在复杂的推理任务下，过程监督(PRM)全面优于结果监督(ORM)。

## 背景知识

奖励模型的作用主要可以用在强化学习中给输出答案打分，PRM的训练过程主要是通过学习输出答案是否符合人类偏好。奖励模型可以分为过程奖励模型和结果奖励模型。
- ORM的主要训练过程可以是让模型通读这篇解答，然后在模型最后一个token处连接一个预测头来判断答案是否准确。
- PRM的主要做法是在每个步骤后都接上预测头，预测目标是人类的打分。

<p align="center">
      <img src="./picture/StepbStep.png" width="100%" />
</p>


## 具体做法

该论文不是讨论如何强化学习训练模型，而是训练性能强的奖励模型，作者做法是通过一个固定的模型生成N个答案，通过奖励模型进行打分，然后比较奖励模型选择正确答案的概率。

- 先通过预训练使模型学会分步输出步骤

- 构建PRM800K数据集：通过生成器生成答案，然后通过人类进行标注是昂贵的，因此，作者采用主动学习的方式，先通过小规模的数据训练PRM模型，但是，如果将随机的答案给人类打分，大部分答案可能是明显的错误，这种数据的价值不高。于是作者将模型拿不准的数据给人类标注员，具体而言可以是答案错误但是PRM分数高的数据，或者是PRM分数低，但是答案错误的数据。

- 通过数据来训练PRM和ORM模型，主要任务是给输出的答案或步骤打分

- 评估方法通过Best-of-N搜索策略，先让生成模型产生多个答案，然后通过奖励模型挑选出最佳答案，比较挑出正确答案的准确率。

## 结论

- 结果奖励模型通常会导致对齐税，就是遵循人类偏好训练会导致模型的基础能力下降

- ORM容易奖励答案正确但是思考步骤错误的数据

- PRM模型能够给予更加高的信息密度，其奖励信号是稠密的，对于模型推理能力能够有更高的帮助。
